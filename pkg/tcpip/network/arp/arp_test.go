...
func TestLinkAddressRequest(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			localAddr:      testAddr,
			remoteLinkAddr: remoteLinkAddr,
			expectErr:      tcpip.ErrBadLocalAddress,
		},
		{
		...
		},
		{
			...
			remoteLinkAddr: remoteLinkAddr,
			expectErr:      tcpip.ErrNetworkUnreachable,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// local address.
			if err := linkRes.LinkAddressRequest(remoteAddr, test.localAddr, test.remoteLinkAddr, &testInterface{LinkEndpoint: linkEP, nicID: nicID}); err != test.expectErr {
				t.Fatalf("got p.LinkAddressRequest(%s, %s, %s, _) = %s, want = %s", remoteAddr, test.localAddr, test.remoteLinkAddr, err, test.expectErr)
			}
			...
			if got := tcpip.Address(rep.ProtocolAddressSender()); got != test.expectLocalAddr {
				t.Errorf("got ProtocolAddressSender = %s, want = %s", got, test.expectLocalAddr)
			}
