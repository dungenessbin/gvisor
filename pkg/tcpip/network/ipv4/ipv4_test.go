...
func TestIPv4Sanity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ICMPType:            header.ICMPv4ParamProblem,
			ICMPCode:            header.ICMPv4UnusedCode,
			paramProblemPointer: header.IPv4MinimumSize + 1,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.shouldFail && !test.expectErrorICMP {
				t.Errorf("unexpected packet response")
			}
			...
			case header.ICMPv4ParamProblem:
				if !test.shouldFail || !test.expectErrorICMP {
					t.Fatalf("got Paramter Problem with pointer %d", replyICMPHeader.Pointer())
				}
				checker.IPv4(t, replyIPHeader,
			...
			case header.ICMPv4DstUnreachable:
				if !test.shouldFail || !test.expectErrorICMP {
					t.Fatalf("got ICMP error packet type %d, code %d",
